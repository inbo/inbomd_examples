...
toc: FALSE
slide_level: 2
cover_photo: e-book-ebook-firewood-1475272-darker.jpg # https://www.pexels.com/photo/person-holding-black-e-book-reader-near-pile-of-firewood-1475272/
flandersfont: TRUE
cover_horizontal: TRUE
reviewer:
  - name:
      given: reviewer_Firstname
      family: reviewer_Lastname
    email: "reviewer.firstname.lastname@inbo.be"
...
## What is INBOmd?
- RMarkdown ([R beginners course](https://inbo.github.io/tutorials/tutorials/r_beginners/)):
  - Simple and easy to use plain text language
  - Combine R code, data analysis results (plots, tables,...) with written commentary
  - Nicely formatted and reproducible document
- Bookdown:
  - Writing books and long-form articles / reports with R markdown 
- INBOmd:
  - Bookdown templates for slides, reports, and posters in the INBO style (colors, font,...)
  - [Color palettes](https://github.com/inbo/INBOtheme)
  - Handy user functions


# Motivation & examples in practice

## Motivation

- Easily use cross references, citations, and bibliography
- Scripts and text interwoven
- New data = No problem!
- INBO look with little effort

## Examples

- Watina (cfr OS Café Joost)
- Reports that stay largely the same but need to be updated with new data / analyses regularly
  - EU habitat reporting
  - Vleemo radarproject
- The same analysis needs to be done for different subsets of the data
  - Generating a seperate report on partridge counts for each Game Management Unit (GMU)
  - Generate a chapter for each habitat in the EU habitat reporting
...
```{r figure-example, warning = FALSE, message = FALSE, fig.cap="Pressure versus temperature", fig.height = 2, fig.width = 4}
library(tidyverse)
library(INBOtheme)
ggplot(pressure) + 
...
[https://forms.gle/qYZnFeDVLPkhC2vFA](
https://forms.gle/qYZnFeDVLPkhC2vFA)


```{r warning = FALSE, message = FALSE, fig.width=2.5, fig.height=2.5}
library(qrcode)
code <- qr_code("https://forms.gle/qYZnFeDVLPkhC2vFA")
plot(code)
```
