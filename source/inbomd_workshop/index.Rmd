...
output:
  bookdown::pdf_book:
    ...
    theme: inboenglish
    flandersfont: TRUE
    cover: e-book-ebook-firewood-1475272-darker.jpg # https://www.pexels.com/photo/person-holding-black-e-book-reader-near-pile-of-firewood-1475272/
...
- Documention on `INBOmd`
    - https://inbomd.netlify.com
- Example code using `INBOmd` 
    - https://inbomd-examples.netlify.com/
    - Do have a look at our [tips and trics for the pdf version](https://inbomd-examples.netlify.com/inbo_rapport/13-lay-out-tips-voor-pdf.html)
...
## Important things to known about bookdown
...
- You can split long documents over multiple `.Rmd` files
    ...
    - All `.Rmd` files will be glued into a single `.Rmd` when rendering the document
        ...
        - Their order: `index.Rmd` + other files in alphabethical order
    - Pro tip: start the filenames with a number indicating the order
    
## Starting a new document

- Open RStudio
- Choose File > New File > R Markdown
- Select 'From Template' and select 'INBO rapport'
- Enter the `name` for your report and select it's main directory and press 'OK'
    - This creates `name/name.Rmd` in the selected directory
    - Rename `name/name.Rmd` to `name/index.Rmd`
- Choose File > New Project > Existing directory, select `name` and press 'Create project'
- Open `_bookdown.yml` in this project and update `book_filename` and `output_dir`
- Open `index.Rmd` in this project and update the YAML header

## Coverting an existing RMarkdown document
...
- The `setup` chunk is the best location to load packages
...
- Open the dropdown menu to select the required output format
    - Defaults to **all** formats listed in the YAML
...
- Chunk setup identical too chunk setup for figures
- First try `knitr::kable()` to generate the table
    - See [examples](https://inbomd-examples.netlify.com/inbo_rapport/7-figuren-en-tabellen.html#tabellen) on the INBOmd website
    - Improve formating with `kableExtra`
...
- Add `bibliography: your-bibliography.bib` to the YAML header
    - Must have `.bib` extention
    - Name contains only letters, digits and dash (`-`)
- Must be in bibtex format
- Add the code below at add a title for the references in HTML or EPUB
...
- `[@bibtex-key]` for the notation with paratheses [@Agresti2002]
- `@bibtex-key` for the notation without paratheses Amano2012
...
- Create a sensible key for each reference in your bibliography
    ...
    - Edit the bibliography using RStudio or JabRef

## Inserting citations using `citr` addin
...
## Using Zotero through `citr` addin
...
- Code between single `$` results in an inline equation $\sum_{i=1}^nx_i$
- Code between double `$` results in a stand alone equation $$\sum_{i=1}^nx_i$$
...
- Headings
    ...
    - Pro-tip: use a prefix to indicate the heading level
        ...
        - `## Conclussion (#s:executive-conclussion)`

## Boxes

- Open the dropdown menu 'Addins' for the RStudio toolbar
...
- For pdf only
    - Place `\mainmatter` at the position where the table of content should go
...
- Appendix chapters will be get a continuing number
...
- Start by creating a regular Rmarkdown subdocument
- Use a name starting with underscore (`_`)
- Create a template tag by placing it between double curly braces `{{` and `}}`.
- Pro tips:
...
```{r echo = FALSE}
title <- "## `r selection$title[1]`"
plot_template <- '```{r mt-{{id}}-plot, fig.cap = selection$caption[1]}
ggplot(selection, aes(x = year, y = value)) + geom_point()
```'
```
...
selection <- filter(mt, id == this_id)
```
...
````markdown
`r ''````{r display-monthly-totals, results = "asis"}
to_do <- unique(mt$id)
rmd <- sapply(
  to_do, 
  function(id) {
